...
function renderWalletTab() {
  // Setup grid — inputs for each owner × method
  const grid = $('walletSetupGrid');
  if (!grid) return;
  grid.innerHTML = OWNERS.flatMap(owner =>
    WALLET_DEFS.map(w => {
      const key = owner+'-'+w.key;
      const val = Number(state.wallets[key]||0);
      return `
        <div class="field">
          <label>${w.icon} ${owner} · ${w.label}</label>
          <input type="number" min="0" id="wallet_${key.replace('-','_')}" placeholder="0" value="${val||''}"/>
        </div>`;
    })
  ).join('');

  // Detail list — current balance per wallet
  const detail = $('walletDetailList');
  if (!detail) return;
  detail.innerHTML = OWNERS.flatMap(owner =>
    WALLET_DEFS.map(w => {
      const key = owner+'-'+w.key;
      const initial = Number(state.wallets[key]||0);
      const bal = walletBalance(owner, w.key);
      const hasTx = state.transactions.some(t => t.owner===owner && (METHOD_MAP[t.method]||t.method)===w.key);
      if (initial===0 && !hasTx) return '';
      const diff = bal - initial;
      return `
        <div class="owner-card">
          ...
          <div>
            ...
            <div style="font-size:12px;color:#aaa">Inicial: ${fmt(initial)}</div>
          </div>
          <div class="owner-stats">
            <div><div class="owner-stat-val ${diff>=0?'pos':'neg'}">${diff>=0?'+':''}${fmt(diff)}</div><div class="owner-stat-label">Movimientos</div></div>
            <div><div class="owner-stat-val ${bal>=0?'':' neg'}" style="font-size:17px">${fmt(bal)}</div><div class="owner-stat-label">Saldo actual</div></div>
          </div>
        </div>`;
    })
  ).filter(Boolean).join('') || '<div class="empty">Configura los saldos iniciales arriba para ver el detalle.</div>';
}
