...
function renderDashboard(){
  ...
  // DEBUG: show what month we're filtering and sample dates
  const ym=new Date().toISOString().slice(0,7);
  const sampleDates=[...new Set(state.transactions.slice(0,5).map(t=>t.date||'sin fecha'))].join(', ');
  const sampleOwners=[...new Set(state.transactions.map(t=>t.owner))].join(', ');
  console.log('DEBUG mes actual:', ym, '| total tx:', state.transactions.length, '| fechas muestra:', sampleDates);
  console.log('DEBUG owners en tx:', sampleOwners);
  console.log('DEBUG mes actual rows:', mr.length, '| byOwnerMonth Alexander:', JSON.stringify(byOwnerMonth('Alexander')));
  console.log('DEBUG mr owners:', [...new Set(mr.map(t=>t.owner))].join(', '));
