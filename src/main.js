...
// Wallets — Hogar has no Tarjeta
const WALLET_DEFS_BY_OWNER = {
  'Alexander': ['Banco','Efectivo','Tarjeta'],
  'Nuri':      ['Banco','Efectivo','Tarjeta'],
  'Hogar':     ['Banco','Efectivo'],
};
const WALLET_META = {
  'Banco':    { label:'Banco',    icon:'🏦', cls:'bank' },
  'Efectivo': { label:'Efectivo', icon:'💵', cls:'cash' },
  'Tarjeta':  { label:'Tarjeta',  icon:'💳', cls:'card' },
};
// Flat list respecting per-owner rules
const WALLET_DEFS = Object.entries(WALLET_DEFS_BY_OWNER).flatMap(([owner, keys]) =>
  keys.map(key => ({ owner, key, ...WALLET_META[key] }))
);

// Calculate current real balance for owner+method — includes traslados
function walletBalance(owner, methodKey) {
  // movements from transactions (starts at zero — no manual initial)
...
function saveTraslado() {
  ...
  const to     = $('trasladoTo').value;
  const amount = Number($('trasladoAmount').value || 0);
  const date   = $('trasladoDate').value || (()=>{const d=new Date();return d.getFullYear()+'-'+String(d.getMonth()+1).padStart(2,'0')+'-'+String(d.getDate()).padStart(2,'0');})();
...
function clearTraslado() {
  ...
  $('trasladoDate').value  = (()=>{const d=new Date();return d.getFullYear()+'-'+String(d.getMonth()+1).padStart(2,'0')+'-'+String(d.getDate()).padStart(2,'0');})();
...
async function loadInitialData() {
  ...
  try {
    ...
    if (snap.exists()) {
      Object.assign(state, normalize(snap.data()));
    } else {
...
function getNavYM(offset=0){const d=new Date();d.setDate(1);d.setMonth(d.getMonth()+offset);return d.getFullYear()+'-'+String(d.getMonth()+1).padStart(2,'0');}
...
function localYM(){const d=new Date();return d.getFullYear()+'-'+String(d.getMonth()+1).padStart(2,'0');}
...
function currentMonthRows(){return state.transactions.filter(t=>(t.date||'').slice(0,7)===localYM());}
...
function clearIncomeForm(){
  $('incomeOwner').value='Alexander';$('incomeDate').value=(()=>{const d=new Date();return d.getFullYear()+'-'+String(d.getMonth()+1).padStart(2,'0')+'-'+String(d.getDate()).padStart(2,'0');})();refreshCategorySelects();
...
function clearExpenseForm(){
  $('expenseType').value='gasto';refreshCategorySelects();$('expenseOwner').value='Hogar';$('expenseDate').value=(()=>{const d=new Date();return d.getFullYear()+'-'+String(d.getMonth()+1).padStart(2,'0')+'-'+String(d.getDate()).padStart(2,'0');})();
...
function renderWalletCards() {
  ...
  if (!container) return;
  const cards = WALLET_DEFS.map(w => ({
    ...w, bal: walletBalance(w.owner, w.key),
    hasTx: state.transactions.some(t => t.owner===w.owner && (METHOD_MAP[t.method]||t.method)===w.key)
         || (state.traslados||[]).some(t => t.owner===w.owner && (t.from===w.key||t.to===w.key))
  })).filter(w => w.bal !== 0 || w.hasTx);
  if (!cards.length) {
    container.innerHTML = '<div class="empty" style="grid-column:1/-1">Registra ingresos y gastos para ver el saldo de cada cuenta.</div>';
    return;
  }
  container.innerHTML = cards.map(w => `
    <div class="wallet-card ${w.cls}">
      <div class="wallet-icon">${w.icon}</div>
      <div class="wallet-label">${w.owner} · ${w.label}</div>
      <div class="wallet-amount ${w.bal<0?'neg':''}">${fmt(w.bal)}</div>
      <div class="wallet-sub">Acumulado total</div>
    </div>`).join('');
}
...
function renderWalletTab() {
  ...
  if (!detail) return;
  const rows = WALLET_DEFS.map(w => {
    const bal   = walletBalance(w.owner, w.key);
    const txIn  = sum(state.transactions.filter(t=>t.owner===w.owner&&(METHOD_MAP[t.method]||t.method)===w.key&&t.type==='ingreso').map(t=>t.amount));
    const txOut = sum(state.transactions.filter(t=>t.owner===w.owner&&(METHOD_MAP[t.method]||t.method)===w.key&&(t.type==='gasto'||(t.type==='deuda'&&t.debtStatus==='pagada'))).map(t=>t.amount));
    const trIn  = sum((state.traslados||[]).filter(t=>t.owner===w.owner&&t.to===w.key).map(t=>t.amount));
    const trOut = sum((state.traslados||[]).filter(t=>t.owner===w.owner&&t.from===w.key).map(t=>t.amount));
    const hasTx = txIn>0||txOut>0||trIn>0||trOut>0;
    return { ...w, bal, txIn, txOut, trIn, trOut, hasTx };
  }).filter(w => w.hasTx || w.bal !== 0);
  if (!rows.length) {
    detail.innerHTML = '<div class="empty">Aún no hay movimientos en ninguna cuenta.</div>';
    return;
  }
  detail.innerHTML = rows.map(w => `
    <div class="owner-card">
      <div class="owner-avatar" style="background:var(--surface-2);font-size:20px">${w.icon}</div>
      <div>
        <div style="font-size:15px;font-weight:800">${w.owner} · ${w.label}</div>
        <div style="font-size:12px;color:#aaa;margin-top:2px">↑ ${fmt(w.txIn+w.trIn)} entradas &nbsp;·&nbsp; ↓ ${fmt(w.txOut+w.trOut)} salidas</div>
      </div>
      <div class="owner-stats">
        <div><div class="owner-stat-val ${w.bal>=0?'pos':'neg'}" style="font-size:18px">${fmt(w.bal)}</div><div class="owner-stat-label">Saldo actual</div></div>
      </div>
    </div>`).join('');
}
