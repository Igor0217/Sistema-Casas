...
// Wallets shown — Hogar has no Tarjeta
const WALLET_DEFS = [
  { owner:'Alexander', key:'Banco',    label:'Banco',    icon:'🏦', cls:'bank' },
  { owner:'Alexander', key:'Efectivo', label:'Efectivo', icon:'💵', cls:'cash' },
  { owner:'Alexander', key:'Tarjeta',  label:'Tarjeta',  icon:'💳', cls:'card' },
  { owner:'Nuri',      key:'Banco',    label:'Banco',    icon:'🏦', cls:'bank' },
  { owner:'Nuri',      key:'Efectivo', label:'Efectivo', icon:'💵', cls:'cash' },
  { owner:'Nuri',      key:'Tarjeta',  label:'Tarjeta',  icon:'💳', cls:'card' },
  { owner:'Hogar',     key:'Banco',    label:'Banco',    icon:'🏦', cls:'bank' },
  { owner:'Hogar',     key:'Efectivo', label:'Efectivo', icon:'💵', cls:'cash' },
];

// Balance = all transactions for this owner+method (no manual initial)
...
async function loadInitialData() {
  ...
  try {
    ...
    // Start real-time listener only after successful initial load
    ...
    if (e.code === 'permission-denied') {
      showToast('⚠️ Sin permiso en Firebase. Abre las reglas de Firestore.', 'error');
    } else {
      showToast('⚠️ No se pudo conectar. Trabajando sin sincronización.', 'error');
    }
  } finally {
    // ALWAYS hide loading and show the app
    showLoading(false);
    render();
  }
}
...
function renderWalletCards() {
  ...
  const cards = WALLET_DEFS.map(w => ({
    ...w,
    bal: walletBalance(w.owner, w.key),
  ...
  if (!cards.length) {
    container.innerHTML = `<div class="empty" style="grid-column:1/-1">Registra ingresos y gastos para ver el saldo de cada cuenta aquí.</div>`;
    return;
  }
...
// ── RENDER: WALLET TAB (Cuentas) ──────────────────────
function renderWalletTab() {
  const detail = $('walletDetailList');
  if (!detail) return;
  const rows = WALLET_DEFS.map(w => {
    const bal   = walletBalance(w.owner, w.key);
    const hasTx = state.transactions.some(t => t.owner===w.owner && (METHOD_MAP[t.method]||t.method)===w.key)
                || (state.traslados||[]).some(t => t.owner===w.owner && (t.from===w.key||t.to===w.key));
    return { ...w, bal, hasTx };
  }).filter(w => w.bal !== 0 || w.hasTx);

  if (!rows.length) {
    detail.innerHTML = '<div class="empty">Aún no hay movimientos registrados en ninguna cuenta.</div>';
    return;
  }
  detail.innerHTML = rows.map(w => {
    // breakdown: ingresos, gastos, traslados in/out
    ...
    return `
      <div class="owner-card">
        ...
        <div><div style="font-size:15px;font-weight:800">${w.owner} · ${w.label}</div>
          <div style="font-size:12px;color:#aaa;margin-top:2px">
            ↑ ${fmt(txIn+trIn)} entradas &nbsp;·&nbsp; ↓ ${fmt(txOut+trOut)} salidas
          </div>
        </div>
        <div class="owner-stats">
